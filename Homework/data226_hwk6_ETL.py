...
def ETL():
    ...
    try:
        ...
        cur.execute("""
                    ...
                    """)
        cur.execute("""
                        CREATE OR REPLACE STAGE USER_DB_DOG.raw.blob_stage
                        url = 's3://s3-geospatial/readonly/'
                        file_format = (type = csv, skip_header = 1, field_optionally_enclosed_by = '"');
                    """)

        cur.execute("""
                        COPY INTO USER_DB_DOG.raw.user_session_channel
                        FROM @USER_DB_DOG.raw.blob_stage/user_session_channel.csv;
                    """)

        cur.execute("""
                        COPY INTO USER_DB_DOG.raw.session_timestamp
                        FROM @USER_DB_DOG.raw.blob_stage/session_timestamp.csv;
                    """)
        cur.execute("COMMIT;")
